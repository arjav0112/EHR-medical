  ...
  return (
    <div>
      <div className="flex items-center gap-5 py-5 border-b border-gray-100">
        <div className="w-16 h-16 rounded-2xl bg-green-600 flex items-center justify-center text-white text-[20px] font-black flex-shrink-0 overflow-hidden">
          {user.photoURL ? <img src={user.photoURL} className="w-16 h-16 object-cover" alt="" /> : initials}
        </div>
        <div>
          <p className="text-[14px] font-bold text-gray-900">{user.displayName || 'Clinician'}</p>
          <p className="text-[12px] text-gray-400 mb-2">{user.email}</p>
          <button className="text-[12px] font-semibold text-green-600 hover:text-green-700 border border-green-200 bg-green-50 px-3 py-1 rounded-lg transition-colors">Change Photo</button>
        </div>
      </div>
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-green-200 bg-green-50 p-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-[18px] font-black text-gray-900">Professional</p>
          <p className="text-[12px] text-gray-500 mt-0.5">Unlimited sessions · $49/month · billed monthly</p>
        </div>
      ...
      <div>
        ...
        {[
          { label: 'Sessions Processed', used: 12, total: 'Unlimited' },
          { label: 'PDF Exports', used: 4, total: 50 },
          { label: 'AI Revisions', used: 7, total: 100 },
      ...
      <div>
        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3">Available Plans</p>
        <div className="grid grid-cols-3 gap-3">
          {[
            { name: 'Starter', price: '$0', desc: '5 sessions/mo', current: false },
            { name: 'Professional', price: '$49', desc: 'Unlimited', current: true },
            { name: 'Enterprise', price: 'Custom', desc: 'Team + EHR', current: false },
          ].map(({ name, price, desc, current }) => (
            <div key={name} className={`rounded-xl border p-4 text-center cursor-pointer transition-all ${current ? 'border-green-400 bg-green-50 ring-2 ring-green-200' : 'border-gray-200 hover:border-gray-300'}`}>
              <p className="text-[12px] font-bold text-gray-700">{name}</p>
              <p className="text-[20px] font-black text-gray-900 mt-1">{price}</p>
              <p className="text-[11px] text-gray-400">{desc}</p>
              {current && <span className="inline-block mt-2 text-[10px] font-bold text-green-600 bg-green-100 px-2 py-0.5 rounded-full">Current</span>}
            </div>
          ))}
        </div>
      </div>
      <div className="flex justify-between pt-2">
        <button className="text-[13px] text-gray-400 hover:text-red-500 transition-colors">Cancel Subscription</button>
        <button className="px-5 py-2.5 rounded-xl text-[13px] font-semibold bg-gray-900 text-white hover:bg-gray-800 transition-all">Download Invoice</button>
      </div>
    </div>
  );
}
      ...
      <div className="pt-4 flex justify-end mb-6">
        <button
          ...
          className={`px-5 py-2.5 rounded-xl text-[13px] font-semibold transition-all ${
            pwdStatus === 'ok' ? 'bg-green-100 text-green-700 border border-green-200' :
    ...
    <div className="space-y-3">
      {integrations.map(({ name, desc, logo, connected }) => (
        <div key={name} className="flex items-center justify-between border border-gray-100 rounded-xl px-4 py-3 hover:border-gray-200 transition-colors">
          ...
          <button className={`text-[12px] font-semibold px-3.5 py-1.5 rounded-lg transition-all ${
            connected
...
export default function SettingsModal({ onClose, initialTab = 'profile' }: { onClose: () => void; initialTab?: Tab }) {
  ...
  const renderTab = () => {
    const u = { displayName: user?.displayName, email: user?.email, photoURL: user?.photoURL, uid: user?.uid };
    switch (activeTab) {
            ...
            {TABS.map(({ id, label, icon }) => (
              <button key={id} onClick={() => setActiveTab(id)}
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-[13px] font-semibold transition-all duration-150 text-left ${
                  activeTab === id ? 'bg-white text-gray-900 shadow-sm border border-gray-100' : 'text-gray-500 hover:text-gray-800 hover:bg-white/60'
                }`}>
